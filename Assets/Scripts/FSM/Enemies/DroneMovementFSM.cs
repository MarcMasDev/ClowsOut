...
public class DroneMovementFSM : FSM_AI
{
    ...
    [SerializeField]
    Transform m_AgentDrone;
    [SerializeField]
    float m_MaxDistanceToRigidBody = 15f;
    float m_DistanceToRigidBody = 0f;
    FollowNavMeshAgentFlying m_FollowNavMeshAgent;
    private float m_timerToRb;
    [SerializeField]
    float m_timeToRecalculatePath = 1f;
    ...
    void Awake()
    {
        ...
        m_FollowNavMeshAgent = GetComponent<FollowNavMeshAgentFlying>();
        Init();
    }
    ...
    void Update()
    {
        ...
        l_posAgent.y = 0;
        m_DistanceToRigidBody = Vector3.Distance(l_pos, l_posAgent);
        if (m_DistanceToRigidBody>= m_MaxDistanceToRigidBody)
        {
    ...
    public override void Init()
    {
        ...
        m_brain.SetOnEnter(States.GOTO_RIGIDBODY, () =>
        {
            m_NavMeshAgent.isStopped = false;
            m_NavMeshAgent.destination = m_FollowNavMeshAgent.GetGoundHitPoint();
            m_timerToRb = 0f;

        }); 
        m_brain.SetOnStay(States.GOTO_RIGIDBODY, () =>
        {
            m_timerToRb += Time.deltaTime;
            if (m_timerToRb > m_timeToRecalculatePath)
            {
                m_timerToRb = 0f;
                m_NavMeshAgent.destination = m_FollowNavMeshAgent.GetGoundHitPoint();
            }
